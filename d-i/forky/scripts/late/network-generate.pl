...
my %CFG = (
    ...
    MANAGED_NETWORK_ETHERNET_IFACE      => 'eth0',
    MANAGED_NETWORK_WIFI_MAC            => '',
    MANAGED_NETWORK_WIFI_IFACE          => 'wifi0',
    MANAGED_NETWORK_HOSTNAME            => 'host',
...
sub usage {
    print STDERR "usage: " . basename($0) . " --input PATH --target-templates DIR --fragments DIR [--state-env PATH]\n";
    exit 1;
}
...
sub parse_args {
    ...
    while (@ARGV) {
        ...
        if ($arg eq '--target-templates' || $arg eq '--fragments') {
            @ARGV || usage();
            my $directory = shift @ARGV;
            fatal("template directory must be an absolute, non-symlink directory")
                if $directory !~ m{\A/} || !-d $directory || -l $directory;
            if ($arg eq '--target-templates') { $TARGET_TEMPLATES = $directory; }
            else { $FRAGMENTS = $directory; }
            next;
        }
        usage();
    }
    usage() if $input eq '' || !defined($TARGET_TEMPLATES) || !defined($FRAGMENTS);
    return $input;
}
...
sub render_native {
    ...
    sysopen my $fh, $path, O_RDONLY | O_NOFOLLOW or fatal("cannot open template $relative: $!");
    my @st = stat($fh);
    fatal("unsafe template $relative") unless @st && S_ISREG($st[2]) && $st[3] == 1
        && $st[4] == $> && ($st[2] & 0022) == 0 && $st[7] <= 262144;
    local $/;
    my $text = <$fh>;
...
sub iface_performance_options {
    my ($metric) = @_;
    return render_native($FRAGMENTS, 'performance.tmpl', METRIC => $metric);
}
...
sub wifi_options {
    ...
    return render_native($FRAGMENTS, "wifi-$security.tmpl",
        SSID => $CFG{MANAGED_NETWORK_WIFI_ESSID},
        KEY => $security eq 'wep' ? $CFG{MANAGED_NETWORK_WIFI_WEP} : $CFG{MANAGED_NETWORK_WIFI_WPA});
}

sub render_iface_stanza {
    my ($link_type, $record, $metric) = @_;
    my $performance = iface_performance_options($metric);
    my $search = $CFG{MANAGED_NETWORK_DOMAIN} eq '' ? '' :
        render_native($FRAGMENTS, 'dns-search.tmpl', DOMAIN => $CFG{MANAGED_NETWORK_DOMAIN});
    my $text = render_native($FRAGMENTS, 'inet4.tmpl',
        LINK_TYPE => $link_type, IFACE => $record->{iface}, MAC => $record->{mac},
        DETECTED_IFACE => $record->{detected_iface}, ADDRESS => $record->{ipv4_address},
        NETMASK => $record->{ipv4_netmask}, GATEWAY => $CFG{MANAGED_NETWORK_IPV4_GATEWAY},
        DNS => $CFG{MANAGED_NETWORK_IPV4_DNS}, SEARCH => $search,
        WIFI => $link_type eq 'wifi' ? wifi_options() : '', PERFORMANCE => $performance);
    if ($CFG{MANAGED_NETWORK_IPV6_ENABLED} eq 'true') {
        $text .= render_native($FRAGMENTS, 'inet6.tmpl',
            IFACE => $record->{iface}, ADDRESS => $record->{ipv6_address},
            NETMASK => $CFG{MANAGED_NETWORK_IPV6_PREFIXLEN}, GATEWAY => $CFG{MANAGED_NETWORK_IPV6_GATEWAY},
            DNS => $CFG{MANAGED_NETWORK_IPV6_DNS}, PERFORMANCE => $performance);
    }
    return $text;
}
...
sub render_link_file {
    ...
    my $name = $link_type eq 'wifi' ? '11-wifi' : '10-ethernet';
    return render_native($TARGET_TEMPLATES, "etc/systemd/network/$name.link.tmpl",
        MAC => $record->{mac}, IFACE => $record->{iface});
}
...
sub render_networkmanager_unmanaged {
    ...
    return render_native($TARGET_TEMPLATES, 'etc/NetworkManager/conf.d/90-network-unmanaged.conf.tmpl',
        MATCHES => join(';', @matches));
}

sub render_interfaces_base {
    return render_native($TARGET_TEMPLATES, 'etc/network/interfaces');
}

sub write_runtime_defaults {
    my ($selected) = @_;
    my %values = map { $_ => $CFG{$_} } keys %CFG;
    $values{MANAGED_NETWORK_LINK_TYPES} = join(' ', link_types());
    $values{MANAGED_NETWORK_WIFI_PSK_SECURITY} = lc($CFG{MANAGED_NETWORK_WIFI_PSK_SECURITY});
    $values{MANAGED_NETWORK_WIFI_ESSID} = '' if !exists($selected->{wifi});
    for my $link_type (qw(ethernet wifi)) {
        my $upper = uc($link_type);
        my $r = $selected->{$link_type} // {};
        $values{"MANAGED_NETWORK_${upper}_IFACE"} = $r->{iface} // $CFG{"MANAGED_NETWORK_${upper}_IFACE"};
        for my $field (qw(mac ipv4_cidr ipv6_cidr)) {
            $values{"MANAGED_NETWORK_${upper}_" . uc($field)} = $r->{$field} // '';
        }
    }
    for my $version (4, 6) {
        for my $kind (qw(host network)) {
            $values{"MANAGED_NETWORK_IPV${version}_" . uc($kind) . '_CIDRS'} = join(' ',
                grep { $_ ne '' } map { $selected->{$_}->{"ipv${version}_${kind}_cidr"} // '' }
                grep { exists $selected->{$_} } link_types());
        }
    }
    # The schema and config body live in the template; only literal values are serialized here.
    for my $key (keys %values) {
        my $assignment = shell_assignment($key, $values{$key});
        $assignment =~ s/\A[^=]+=//;
        chomp $assignment;
        $values{$key} = $assignment;
    }
    write_file_atomic(target_path('/etc/network/host.conf'), 0600,
        render_native($TARGET_TEMPLATES, 'etc/network/host.conf.tmpl', %values));
}
...
sub main {
    ...
    write_file_atomic(
        target_path('/etc/network/interfaces.d/50-network'),
        exists($selected{wifi}) ? 0600 : 0644,
        render_native($TARGET_TEMPLATES, 'etc/network/interfaces.d/50-network.tmpl', STANZAS => join("\n", @parts))
    );
    ...
    for my $link_type (@link_file_types) {
        my $link_path = $link_type eq 'wifi'
            ? '/etc/systemd/network/11-wifi.link'
    ...
    write_file_atomic(
        target_path('/etc/NetworkManager/conf.d/90-network-unmanaged.conf'),
        0644,
