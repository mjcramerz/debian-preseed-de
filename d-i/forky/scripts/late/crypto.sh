...
crypto_write_config() {
  ...
  crypto_render_target_template \
    "$(installer_repo_join_var DIR_SCRIPTS_FIRSTBOOT assets/var/lib/tpm2-enrollment/config.env.tmpl)" \
...
crypto_write_crypttab() {
  # Keep unrelated d-i entries; render root/home rows from the native template.
  # Never truncate the authoritative file before the complete replacement exists.
  ...
  crypttab_template=$(mktemp "${tmp_env_dir}/crypttab-template.XXXXXX") || crypto_fatal "cannot stage crypttab template"
  crypttab_records=$(mktemp "${tmp_env_dir}/crypttab-records.XXXXXX") || crypto_fatal "cannot stage crypttab records"
  bootstrap_fetch_seed_file "$seed_base" \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/crypttab.tmpl)" "$crypttab_template" 0600 "crypttab template"
  installer_apply_scalar_placeholders "$crypttab_template" "$crypttab_records" \
    ROOT_CRYPT_NAME "$ROOT_CRYPT_NAME" ROOT_LUKS_UUID "$root_luks_uuid" \
    HOME_CRYPT_NAME "$HOME_CRYPT_NAME" HOME_LUKS_UUID "$home_luks_uuid" HOME_KEY_FILE "$home_key_target" ||
    crypto_fatal "cannot render crypttab records"
  installer_assert_no_unresolved_installer_placeholders "$crypttab_records" crypttab
  crypttab_publish=$(mktemp "${target_root}/etc/.crypttab.XXXXXX") || crypto_fatal "cannot stage crypttab"
  [ ! -L "$crypttab_path" ] || crypto_fatal "refusing a symlinked crypttab"
  if ! (cat "$crypttab_tmp" "$crypttab_records" >"$crypttab_publish" &&
        chmod 0600 "$crypttab_publish" && mv -fT -- "$crypttab_publish" "$crypttab_path"); then
    rm -f -- "$crypttab_publish" "$crypttab_template" "$crypttab_records" "$crypttab_tmp"
    crypto_fatal "cannot publish crypttab"
  fi
  rm -f -- "$crypttab_template" "$crypttab_records" "$crypttab_tmp"
}
...
crypto_stage_target_asset \
  ...
  0755
crypto_stage_target_asset \
  "$(installer_repo_join_var DIR_SCRIPTS_FIRSTBOOT assets/usr/local/bin/tpm2-enroll-launch)" \
  /usr/local/bin/tpm2-enroll-launch \
  0755
crypto_stage_target_asset \
  "$(installer_repo_join_var DIR_SCRIPTS_FIRSTBOOT assets/etc/profile.d/tpm2-enroll-prompt.sh)" \
  /etc/profile.d/tpm2-enroll-prompt.sh \
  0644
crypto_stage_target_asset \
  "$(installer_repo_join_var DIR_SCRIPTS_FIRSTBOOT assets/etc/xdg/autostart/tpm2-enroll.desktop)" \
