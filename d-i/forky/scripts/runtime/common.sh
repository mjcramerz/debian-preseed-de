...
runtime_preseed_env_value() (
  runtime_preseed_env_key=$1
  case "$runtime_preseed_env_key" in
    primary_user|primary_password|primary_gpg_passphrase|root_password) ;;
    *) return 1 ;;
  esac

  runtime_preseed_env_file=${INSTALLER_PRESEED_ENV_FILE:-/preseed.env}
  case "$runtime_preseed_env_file" in
    /*) ;;
    *) return 1 ;;
  esac
  [ -f "$runtime_preseed_env_file" ] || return 1
  [ ! -L "$runtime_preseed_env_file" ] || return 1
  [ -r "$runtime_preseed_env_file" ] || return 1

  # This file is sourced as code. Accept only the invoking account's private
  # initrd file (root in d-i), never a readable shared or user-owned credential.
  runtime_preseed_env_owner=$(id -u) || return 1
  runtime_preseed_env_metadata=$(stat -c '%u:%a' "$runtime_preseed_env_file" 2>/dev/null) || return 1
  case "$runtime_preseed_env_metadata" in
    "$runtime_preseed_env_owner:400"|"$runtime_preseed_env_owner:600") ;;
    *) return 1 ;;
  esac
  ...
  # Keep standalone account rendering aligned with the shared installer
  # resolver without exporting or persisting the initrd-owned secret values.
  # shellcheck disable=SC1090
  . "$runtime_preseed_env_file" || return 1

  case "$runtime_preseed_env_key" in
    primary_user)
      runtime_preseed_env_result=${PRESEED_PRIMARY_USERNAME:-}
      ;;
    primary_password)
      runtime_preseed_env_result=${PRESEED_PRIMARY_PASSWORD:-}
      ;;
    primary_gpg_passphrase)
      runtime_preseed_env_result=${PRESEED_PRIMARY_GPG_PASSPHRASE:-}
      ;;
    root_password)
      runtime_preseed_env_result=${PRESEED_ROOT_PASSWORD:-}
      ;;
  esac

  [ -n "$runtime_preseed_env_result" ] || return 1
  printf '%s\n' "$runtime_preseed_env_result"
)
...
runtime_debconf_command() (
  ...
  command -v debconf-communicate >/dev/null 2>&1 ||
    runtime_fatal "no debconf interface is available to seed ${runtime_debconf_question}"

  # Both the process status and the debconf protocol status must succeed.
  # Never print the request or response: either may contain a password.
  runtime_debconf_reply=$(printf '%s\n' "$runtime_debconf_request" |
    debconf-communicate 2>/dev/null) ||
    runtime_fatal "debconf command failed for ${runtime_debconf_question}"
  case "$runtime_debconf_reply" in
    0|0\ *) return 0 ;;
    *) runtime_fatal "debconf rejected command for ${runtime_debconf_question}" ;;
  esac
)
...
runtime_apply_answers_file() {
  ...
  if command -v debconf-set-selections >/dev/null 2>&1; then
    # A terminal backslash is a continuation marker to debconf-set-selections,
    # but may be literal password data in our single-physical-line fragments.
    # Register those questions with empty values, then send their exact values
    # through the debconf protocol. Never install a synthetic password instead.
    if LC_ALL=C grep -q '\\$' "$path"; then
      command -v debconf-communicate >/dev/null 2>&1 ||
        runtime_fatal "debconf-communicate is required for literal trailing backslashes"
      ...
      done <"$path" | debconf-set-selections ||
        runtime_fatal "debconf failed to register answer fragment: ${path}"
      runtime_seed_answers_file "$path" || return $?
    else
      debconf-set-selections "$path" ||
        runtime_fatal "debconf failed to apply answer fragment: ${path}"
    fi
    return 0
  fi
