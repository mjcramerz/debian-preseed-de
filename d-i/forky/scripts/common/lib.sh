...
installer_preseed_env_value() (
  installer_preseed_env_key=$1

  case "$installer_preseed_env_key" in
    netcfg/wireless_wpa|wireless_wpa|wifi_wpa|\
    fruux_username|fruux_password|\
  ...
  installer_preseed_env_file=${INSTALLER_PRESEED_ENV_FILE:-/preseed.env}
  case "$installer_preseed_env_file" in
    /*) ;;
    *) return 1 ;;
  esac
  [ -f "$installer_preseed_env_file" ] || return 1
  [ ! -L "$installer_preseed_env_file" ] || return 1
  [ -r "$installer_preseed_env_file" ] || return 1
  ...
  installer_preseed_env_owner=$(id -u) || return 1
  installer_preseed_env_metadata=$(stat -c '%u:%a' "$installer_preseed_env_file" 2>/dev/null) || return 1
  case "$installer_preseed_env_metadata" in
    "$installer_preseed_env_owner:400"|"$installer_preseed_env_owner:600") ;;
    *) return 1 ;;
  esac
  ...
  # /preseed.env is a trusted initrd-owned shell assignment file. Source it in
  # this subshell so its values cannot alter the calling installer's state.
  # shellcheck disable=SC1090
  . "$installer_preseed_env_file" || return 1

  case "$installer_preseed_env_key" in
    netcfg/wireless_wpa|wireless_wpa|wifi_wpa)
      installer_preseed_env_result=${PRESEED_WIFI_PASSPHRASE:-}
      ;;
    fruux_username)
      installer_preseed_env_result=${PRESEED_FRUUX_USERNAME:-}
      ;;
    fruux_password)
      installer_preseed_env_result=${PRESEED_FRUUX_PASSWORD:-}
      ;;
    primary_user)
      installer_preseed_env_result=${PRESEED_PRIMARY_USERNAME:-}
      ;;
    primary_password)
      installer_preseed_env_result=${PRESEED_PRIMARY_PASSWORD:-}
      ;;
    primary_gpg_passphrase)
      installer_preseed_env_result=${PRESEED_PRIMARY_GPG_PASSPHRASE:-}
      ;;
    root_password)
      installer_preseed_env_result=${PRESEED_ROOT_PASSWORD:-}
      ;;
    crowdsec_token|crowdsec_enroll_token|crowdsec_attachment_key)
      installer_preseed_env_result=${PRESEED_CROWDSEC_TOKEN:-}
      ;;
    tailscale_authkey|tailscale_auth_key)
      installer_preseed_env_result=${PRESEED_TAILSCALE_TOKEN:-}
      ;;
    telegram_chat_id)
      installer_preseed_env_result=${PRESEED_TELEGRAM_CHAT_ID:-}
      ;;
    telegram_api_key)
      installer_preseed_env_result=${PRESEED_TELEGRAM_API_KEY:-}
      ;;
    cf_r2_access_key)
      installer_preseed_env_result=${PRESEED_CF_APTLY_ACCESS_KEY:-}
      ;;
    cf_r2_secret_key)
      installer_preseed_env_result=${PRESEED_CF_APTLY_SECRET_KEY:-}
      ;;
    obs_username)
      installer_preseed_env_result=${PRESEED_OBS_USERNAME:-}
      ;;
    obs_password)
      installer_preseed_env_result=${PRESEED_OBS_PASSWORD:-}
      ;;
  esac

  [ -n "$installer_preseed_env_result" ] || return 1
  printf '%s\n' "$installer_preseed_env_result"
)
