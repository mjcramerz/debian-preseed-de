...
desktop_validate_managed_app_default_exec() {
  ...
  [ "$1" = /usr/local/bin/labwc-app ] ||
    desktop_fatal "${var_name} must use /usr/local/bin/labwc-app"
  case "$2" in
    launch|pure-privacy) ;;
    intel)
...
# Connector classification is profile-owned and shared by Waybar and Fuzzel.
# Validate before splitting, so shell glob expansion can never reinterpret it.
desktop_validate_internal_output_prefixes() (
  prefixes=${LABWC_OUTPUT_INTERNAL_PREFIXES:-}
  case "$prefixes" in
    ''|*[!ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789._\ -]*)
      desktop_fatal "invalid LABWC_OUTPUT_INTERNAL_PREFIXES" ;;
  esac
  set -- $prefixes
  [ "$#" -gt 0 ] || desktop_fatal "LABWC_OUTPUT_INTERNAL_PREFIXES must not be blank"
  for prefix do
    case "$prefix" in
      [!ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789]*)
        desktop_fatal "internal output prefix must start with an ASCII letter or digit" ;;
    esac
    [ "${#prefix}" -le 32 ] || desktop_fatal "internal output prefix exceeds 32 characters"
  done
)
...
desktop_validate_policy_env() {
  ...
  desktop_validate_font_family LABWC_TERMINAL_FONT_FAMILY "${LABWC_TERMINAL_FONT_FAMILY}"
  ...
  desktop_validate_absolute_path LABWC_DESKTOP_DEFAULTS_FILE "${LABWC_DESKTOP_DEFAULTS_FILE:-/etc/labwc/desktop.conf}"
  desktop_validate_absolute_path LABWC_DESKTOP_SESSION_COMMAND "${LABWC_DESKTOP_SESSION_COMMAND:-/usr/local/bin/labwc-session}"
  desktop_validate_absolute_path LABWC_WALLPAPER_PATH "${LABWC_WALLPAPER_PATH:-__THEME_TARGET_PATH_LABWC_WALLPAPER_DESKTOP_SWAYBG__}"
  desktop_validate_absolute_path LABWC_LOCK_BACKGROUND_PATH "${LABWC_LOCK_BACKGROUND_PATH:-__THEME_TARGET_PATH_LABWC_WALLPAPER_LOCKSCREEN_SWAYLOCK__}"
  desktop_validate_absolute_path LABWC_GREETER_BACKGROUND_PATH "${LABWC_GREETER_BACKGROUND_PATH:-__THEME_TARGET_PATH_LABWC_WALLPAPER_LOGIN_GTKGREET__}"
  desktop_validate_managed_app_default_exec \
  ...
  desktop_validate_theme_name LABWC_DESKTOP_GTK_ICON_THEME "${LABWC_DESKTOP_GTK_ICON_THEME}"
...
# Shared launcher/menu geometry. DEFAULT is deliberate, never an alias for an
# unknown output being misclassified as external. Only fixed keys reach eval.
desktop_validate_fuzzel_geometry() (
  for fz_class in INTERNAL EXTERNAL DEFAULT; do
    for fz_spec in FONT_SIZE:8:32 HORIZONTAL_PADDING:0:64 VERTICAL_PADDING:0:64 \
        INNER_PADDING:0:64 LINE_HEIGHT:8:64 BORDER_WIDTH:0:32 BORDER_RADIUS:0:128 \
        LAUNCHER_WIDTH:20:200 LAUNCHER_LINES:4:40 MENU_WIDTH:20:200 MENU_LINES:4:40; do
      fz_key=${fz_spec%%:*}
      fz_bounds=${fz_spec#*:}
      case "$fz_key" in
        LAUNCHER_*|MENU_*) fz_name="FUZZEL_${fz_key%%_*}_${fz_class}_${fz_key#*_}" ;;
        *) fz_name="FUZZEL_${fz_class}_${fz_key}" ;;
      esac
      eval "fz_value=\${${fz_name}-}"
      case "$fz_value" in
        ''|*[!0-9]*|0[0-9]*) desktop_fatal "noncanonical Fuzzel geometry: $fz_name" ;;
      esac
      [ "${#fz_value}" -le 3 ] || desktop_fatal "Fuzzel geometry exceeds numeric budget: $fz_name"
      desktop_validate_uint_range "$fz_name" "$fz_value" "${fz_bounds%%:*}" "${fz_bounds#*:}"
    done
  done
)
