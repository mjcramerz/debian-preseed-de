...
require_readable /usr/local/share/doc/git/README.md
require_mode /usr/local/share/doc/git/README.md 644
...
require_readable /etc/ssh/git_known_hosts
require_mode /etc/ssh/git_known_hosts 644
require_readable /usr/local/libexec/ssh-checks
require_mode /usr/local/libexec/ssh-checks 644
...
for path in \
  ...
  /usr/local/libexec/rsyslog-size-rotate \
  ...
  /etc/systemd/system/clamav-signature-update.service \
  /etc/systemd/system/clamav-signature-update.timer \
  ...
  /usr/local/share/nmap/scripts/admin-surface-policy.nse \
  /usr/local/share/nmap/scripts/approved-services.nse \
  /usr/local/share/nmap/scripts/database-exposure-policy.nse \
  /usr/local/share/nmap/scripts/http-security-headers.nse \
  /usr/local/share/nmap/scripts/name-resolution-policy.nse \
  /usr/local/share/nmap/scripts/plaintext-service-policy.nse \
  /usr/local/share/nmap/scripts/service-inventory.nse \
  /usr/local/share/nmap/scripts/tls-service-policy.nse \
  ...
  /opt/glibc/2.44-1/satty/.release \
  ...
  /usr/local/share/applications/computer-management.desktop \
  /usr/local/share/applications/remote-desktop-management.desktop \
...
for background_file in \
  __THEME_TARGET_PATH_LABWC_WALLPAPER_DESKTOP_SWAYBG__ \
  __THEME_TARGET_PATH_LABWC_WALLPAPER_LOCKSCREEN_SWAYLOCK__ \
  __THEME_TARGET_PATH_LABWC_WALLPAPER_LOGIN_GTKGREET__
do
...
for path in \
  ...
  /etc/skel-desktop/Syncthing/obsidian-md/.obsidian/snippets/ux.css \
...
for path in \
  ...
  /usr/local/libexec/clamav-signature-update \
...
require_absent __THEME_TARGET_PATH_LABWC_WALLPAPER_ARCHIVE__
...
for legacy_user_unit in \
  ...
  apt-repo-local-software-notify.service \
  apt-repo-local-software-notify.path \
...
require_readable /usr/local/lib/android-sdk/platform-tools/.release
...
require_readable /usr/local/lib/samloader/.release
...
if [ -r /usr/share/dbus-1/system-services/org.freedesktop.nm_dispatcher.service ]; then
  ...
  nm_dispatcher_unit=
  require_readable /etc/systemd/system/NetworkManager.service.d/20-dispatcher.conf
  require_readable /etc/systemd/system/NetworkManager-dispatcher.service.d/20-persistent.conf
...
for path in \
  ...
  /etc/skel-desktop/.local/share/GottCode/FocusWriter/Themes/word.theme \
  ...
  /etc/skel-desktop/.config/fuzzel/computer-management.ini \
  ...
  /usr/local/share/applications/show-desktop.desktop \
  /usr/local/bin/labwc-health-notify \
  ...
  /etc/skel-desktop/.config/systemd/user/apt-repo-local-software-notify.service \
  /etc/skel-desktop/.config/systemd/user/apt-repo-local-software-notify.path \
...
check_required_owned "$account_home/.local/share/ssh/private/id_git_ed25519"
[ ! -L "$account_home/.local/share/ssh/private/id_git_ed25519" ] || fatal "symlinked managed Git file"
[ "$(find -P "$account_home/.local/share/ssh/private/id_git_ed25519" -maxdepth 0 -printf "%m:%n")" = 600:1 ] || fatal "unsafe managed Git file metadata"
check_required_owned "$account_home/.local/share/ssh/git-key-passphrase.gpg"
[ ! -L "$account_home/.local/share/ssh/git-key-passphrase.gpg" ] || fatal "symlinked managed Git file"
[ "$(find -P "$account_home/.local/share/ssh/git-key-passphrase.gpg" -maxdepth 0 -printf "%m:%n")" = 600:1 ] || fatal "unsafe managed Git file metadata"
...
check_required_owned_dir "$account_home/.local/share/ssh"
[ ! -L "$account_home/.local/share/ssh" ] || fatal "symlinked managed Git directory"
[ "$(find -P "$account_home/.local/share/ssh" -maxdepth 0 -printf "%m")" = 700 ] || fatal "unsafe managed Git directory"
check_required_owned_dir "$account_home/.local/share/ssh/private"
[ ! -L "$account_home/.local/share/ssh/private" ] || fatal "symlinked managed Git directory"
[ "$(find -P "$account_home/.local/share/ssh/private" -maxdepth 0 -printf "%m")" = 700 ] || fatal "unsafe managed Git directory"
...
for path in \
  ...
  "$account_home/Syncthing/obsidian-md/.obsidian/snippets/ux.css" \
...
if [ -r /etc/skel-desktop/.config/systemd/user/apt-repo-local-software-notify.path ]; then
  verify_skeleton_session_link apt-repo-local-software-notify.path
  verify_account_session_link apt-repo-local-software-notify.path
fi
...
for path in \
  ...
  "$account_home/.local/share/GottCode/FocusWriter/Themes/word.theme" \
  ...
  "$account_home/.config/fuzzel/computer-management.ini" \
...
printf "desktop_primary_account_verification user=%s home=%s required_files=%s optional_files=%s optional_missing=%s shell=%s shell_status=%s\n" \
  ...
  "$account_shell" \
  "$shell_status"
' sh "$ACCOUNT_USERNAME" "$ACCOUNT_HOME" "${LABWC_DESKTOP_GTK_ICON_THEME}"
}
...
for config in config_roots:
    ...
    require(osd.get("thumbnailLabelFormat") == "%n __THEME_LABWC_TASKVIEW_LABEL_SEPARATOR_ICON_GLYPH__ %T  %S  %o", "invalid switcher label")
    ...
    require(isinstance(bars, list) and len(bars) == 2
            and [bar.get("name") for bar in bars] == ["internal", "external"],
            "Waybar must contain exactly the internal/external bar definitions")
    for bar in bars:
...
drawer_icons = {
    "custom/app-terminal": "__THEME_WAYBAR_BUTTON_TERMINAL_NORMAL_ICON_PATH__",
    "custom/app-files": "__THEME_WAYBAR_BUTTON_FILES_NORMAL_ICON_PATH__",
    "custom/app-tuta": "__THEME_WAYBAR_BUTTON_TUTANOTA_NORMAL_ICON_PATH__",
    "custom/app-notes": "__THEME_WAYBAR_BUTTON_NOTES_NORMAL_ICON_PATH__",
    "custom/app-sleek": "__THEME_WAYBAR_BUTTON_SLEEK_NORMAL_ICON_PATH__",
}
...
for base in (Path("/etc/skel-desktop"), Path(sys.argv[1])):
    ...
    require(isinstance(bars, list) and len(bars) == 2
            and all(isinstance(bar, dict) for bar in bars)
            and [bar.get("name") for bar in bars] == ["internal", "external"],
            "native drawer requires both output-class layouts")
    common = (config / "style.css").read_text()
    for output_class, size, fill in (("internal", sys.argv[4], sys.argv[5]),
                                     ("external", sys.argv[6], sys.argv[7])):
        require(re.fullmatch(r"[1-9][0-9]{0,2}", size) and 8 <= int(size) <= 128,
                "invalid native drawer icon geometry")
        require(re.fullmatch(r"[0-9]{1,3}", fill) and 0 <= int(fill) <= 100,
                "invalid native drawer background geometry")
        expected_size = size + "px " + size + "px, " + fill + "% " + fill + "%"
        rules = re.findall(r"([^{}]+)\{([^{}]*)\}", re.sub(r"/\*.*?\*/", "", common, flags=re.S))
        for module in ("custom/apps", "custom/wayscriber", *drawer_icons):
            for suffix in ("", ":hover"):
                selector = "#" + module.replace("/", "-")
                # Only these module selectors participate in drawer geometry.
                # Respect specificity: the bar-qualified rule outranks a shared
                # rule, even if the latter occurs later in the file.
                candidates = []
                for order, (selectors, body) in enumerate(rules):
                    for part in (part.strip() for part in selectors.split(",")):
                        for state in (("", ":hover") if suffix else ("",)):
                            for scoped in (False, True):
                                expected = ("window#waybar." + output_class + " " if scoped else "") + selector + state
                                if part == expected:
                                    for value in re.findall(r"background-size:\s*([^;]+);", body):
                                        candidates.append((scoped, bool(state), order, value.strip()))
                require(candidates and max(candidates, key=lambda row: row[:3])[3] == expected_size,
                        "native drawer icon size is misconfigured: " + selector + suffix)
        for module, icon in drawer_icons.items():
            selector = "#" + module.replace("/", "-")
            ...
            # A selector can legitimately have several rules (shared geometry,
            # per-button palette, then artwork). Honor CSS declaration order and
            # reject a later background shorthand that would remove the image.
            backgrounds = [value.strip() for body in normal
                           for value in re.findall(r"(?:^|;)\s*background(?:-image)?\s*:\s*([^;]+)", body)]
            require(backgrounds and "url(\"" + icon + "\")" in backgrounds[-1],
...
print("desktop_native_drawer_verification layouts=2 icons=package-artwork size=profile")
' "$ACCOUNT_HOME" "$ACCOUNT_USERNAME" "$desktop_drawer_software" \
  "$LABWC_WAYBAR_INTERNAL_APP_ICON_SIZE" "$LABWC_WAYBAR_INTERNAL_BACKGROUND_FILL_PERCENT" \
  "$LABWC_WAYBAR_EXTERNAL_APP_ICON_SIZE" "$LABWC_WAYBAR_EXTERNAL_BACKGROUND_FILL_PERCENT"
}
...
row_spacing, icon_size, icon_width = sys.argv[4:7]
account = pwd.getpwnam(sys.argv[2])
        ...
        for module, name, event in (("custom/tomat", "tomat", "on-click-right"),
            ...
            for obj in objects:
                ...
                if cls == "GtkMenu":
                    ...
                    require(box is not None and box.findtext("./property[@name=\"spacing\"]") == row_spacing
                    ...
                    require(props.get("visible") == "True" and props.get("pixel-size") == icon_size
                            and props.get("width-request") == icon_width and props.get("use-fallback") == "True"
                            and re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.+-]*", props.get("icon-name", "")),
...
' "$ACCOUNT_HOME" "$ACCOUNT_USERNAME" "[${desktop_native_menu_modules_left}]" \
    "$LABWC_WAYBAR_INTERNAL_MENU_ROW_SPACING" "$LABWC_WAYBAR_INTERNAL_MENU_ICON_SIZE" \
    "$LABWC_WAYBAR_INTERNAL_MENU_ICON_WIDTH"
}
