...
class TutaIntegrationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.desktop.write_text('[Desktop Entry]\nType=Application\nExec=labwc-managed-app launch tutanota %U\n')
    ...
    def test_persistent_private_seed_is_idempotent_and_host_is_unchanged(self):
        self.prepare()
        self.assertEqual((self.private / 'config/mimeapps.list').read_bytes(), self.mime.read_bytes())
        ...
        self.prepare()
        self.assertEqual(remember.read_text(), 'remembered application state')
        self.assertEqual((self.private / 'applications/tutanota-desktop.desktop').read_text(), 'Tuta-owned integration')
        self.assertIn('labwc-managed-app', self.desktop.read_text())
        self.assertEqual(self.account.read_text(), 'account data must survive')
    ...
    def test_mime_file_can_be_atomically_replaced(self):
        ...
        self.assertEqual((self.private / 'config/mimeapps.list').read_text(), 'new private MIME state')
        self.assertIn('[Default Applications]', self.mime.read_text())
    ...
    def test_absent_seed_is_not_replaced_with_invalid_empty_desktop(self):
        self.desktop.unlink()
        self.prepare()
        self.assertFalse((self.private / 'applications/tutanota-desktop.desktop').exists())

    def test_private_directories_and_seed_modes(self):
        self.prepare()
        for source, _destination in profiles.TUTA_INTEGRATION_DIRECTORY_BINDS:
            self.assertEqual(stat.S_IMODE((self.home / source).stat().st_mode), 0o700)
        self.assertEqual(stat.S_IMODE((self.private / 'config/mimeapps.list').stat().st_mode), 0o600)
    ...
    def test_mount_order_does_not_hide_account_state_or_bind_host_mime_inode(self):
        ...
        source = (PACKAGE / 'labwc_managed_app/sandbox.py').read_text()
    ...
    def test_shared_ipc_directory_is_stable_private_and_rejects_symlinks(self):
        ...
        self.assertEqual((path / 'cookie').read_text(), 'live singleton')
...
class MailDefaultsTests(unittest.TestCase):
    ...
    def test_keeps_non_tuta_rfc822_handler(self):
        ...
        self.repair()
        self.assertIn('message/rfc822=other-mail.desktop;', self.mime.read_text())

    def test_missing_mime_file_created(self):
        self.assertTrue(self.repair())
        self.assertFalse(self.repair())
        self.assertEqual(stat.S_IMODE(self.mime.stat().st_mode), 0o600)
    ...
    def test_symlink_and_malformed_input_are_not_replaced(self):
        ...
        with self.assertRaises(configparser.Error):
            self.repair()
        self.assertEqual(self.mime.read_text(), 'broken configuration without a section')
    ...
    def test_seed_desktop_and_mime_defaults_use_the_same_supported_schemes(self):
        desktop = (TARGET / 'etc/skel-desktop/.local/share/applications/tutanota-desktop.desktop').read_text()
        self.assertIn('MimeType=x-scheme-handler/mailto;x-scheme-handler/tuta;', desktop)
        self.assertIn('tutanota-desktop.desktop', (TARGET / 'etc/xdg/mimeapps.list').read_text())
        self.assertNotIn('message/rfc822=tuta', (TARGET / 'etc/skel-desktop/.config/mimeapps.list').read_text())
...
class ServiceAndNotificationTests(unittest.TestCase):
    ...
    def test_activation_token_is_opaque_bounded_and_not_saved_as_restore_argument(self):
        ...
        self.assertIn('environment.update(desktop_activation_environment())',
                      (PACKAGE / 'labwc_managed_app/session.py').read_text())
        self.assertIn('env: dict[str, str] = desktop_activation_environment()',
                      (PACKAGE / 'labwc_managed_app/environment.py').read_text())
    ...
    def test_mako_invokes_client_callback_instead_of_launching_a_new_generic_mail_window(self):
        config=(TARGET/'etc/skel-desktop/.config/mako/config').read_text()
    ...
    def test_remote_and_podman_terminal_helpers_wait_and_remote_returns_to_submenu(self):
        ...
        source=(TARGET/'usr/local/bin/labwc-podman-menu').read_text()
    ...
    def test_wireshark_waits_for_setsids_child(self):
        source = (TARGET/'usr/local/lib/perl5/site_perl/labwc-network-scan-action/LabwcNetworkScanAction/Client.pm').read_text()
        ...
        result = subprocess.run(['setsid', '-f', '--wait', '/bin/sh', '-c', 'exit 7'], check=False)
...
class PolicyCoverageTests(unittest.TestCase):
    def test_terminal_grants_include_privileged_workers_without_owner_restriction(self):
        rule=(TARGET/'etc/apparmor.d/abstractions/managed-wrapper-terminal').read_text()
        self.assertIn('\n/dev/pts/[0-9]* rw,',rule)
        self.assertNotIn('\nowner /dev/pts/',rule)
        text=(TARGET/'etc/apparmor.d/managed-desktop-wrappers').read_text()
        for name in re.findall(r'^profile (managed-labwc-[\w-]*action(?:-root|-worker)?) ',text,re.M):
            block=text.split('profile '+name+' ',1)[1].split('\nprofile ',1)[0]
            self.assertIn('#include <abstractions/managed-wrapper-terminal>',block,name)
        self.assertIn('etc/apparmor.d/abstractions/managed-wrapper-terminal',
                      (FORKY/'scripts/late/security.sh').read_text())

    def test_readonly_process_inspection_is_bilateral(self):
        text=(TARGET/'etc/apparmor.d/managed-system-wrappers').read_text()
        self.assertIn('ptrace (read) peer=managed-crowdsec-firstboot,',text)
        self.assertIn('ptrace (readby) peer=managed-firstboot,',text)
        self.assertNotIn('ptrace (trace) peer=managed-crowdsec-firstboot,',text)
    ...
    def test_ncdu_fix_is_directory_only_in_existing_file_manager_domain(self):
        ...
        block=text.split('profile managed-desktop-launcher ',1)[1].split('\nprofile ',1)[0]
        self.assertIn('\n  /**/ r,',block)
        self.assertNotIn('\n  /** r,',block)
        self.assertNotIn('\n  /** rw',block)
    ...
    def test_tuta_ipc_path_is_allowed_outside_and_inside_bubblewrap(self):
        text=(TARGET/'etc/apparmor.d/managed-desktop-wrappers').read_text()
        block=text.split('profile managed-labwc-managed-app ',1)[1].split('\nprofile ',1)[0]
