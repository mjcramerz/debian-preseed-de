...
desktop_render_labwc_default_config() {
  ...
  desktop_validate_bool \
    ...
    "$LABWC_NVIDIA_ACCELERATION_AVAILABLE"
  defaults_path=${LABWC_DESKTOP_DEFAULTS_FILE:-/etc/labwc/desktop.conf}
  primary_account_ids=$(desktop_primary_account_ids "${ACCOUNT_USERNAME:-}" || true)
  ...
  desktop_render_role_target_template \
    ...
    LABWC_WALLPAPER_PATH "$(desktop_shell_config_value "${LABWC_WALLPAPER_PATH}")" \
    LABWC_LOCK_BACKGROUND_PATH "$(desktop_shell_config_value "${LABWC_LOCK_BACKGROUND_PATH}")" \
    LABWC_GREETER_BACKGROUND_PATH "$(desktop_shell_config_value "${LABWC_GREETER_BACKGROUND_PATH}")" \
    ...
    LABWC_TERMINAL_FONT_FAMILY "$(desktop_shell_config_value "${LABWC_TERMINAL_FONT_FAMILY}")" \
    ...
    FUZZEL_INTERNAL_FONT_SIZE "$(desktop_shell_config_value "$FUZZEL_INTERNAL_FONT_SIZE")" \
    FUZZEL_INTERNAL_HORIZONTAL_PADDING "$(desktop_shell_config_value "$FUZZEL_INTERNAL_HORIZONTAL_PADDING")" \
    FUZZEL_INTERNAL_VERTICAL_PADDING "$(desktop_shell_config_value "$FUZZEL_INTERNAL_VERTICAL_PADDING")" \
    FUZZEL_INTERNAL_INNER_PADDING "$(desktop_shell_config_value "$FUZZEL_INTERNAL_INNER_PADDING")" \
    FUZZEL_INTERNAL_LINE_HEIGHT "$(desktop_shell_config_value "$FUZZEL_INTERNAL_LINE_HEIGHT")" \
    FUZZEL_INTERNAL_BORDER_WIDTH "$(desktop_shell_config_value "$FUZZEL_INTERNAL_BORDER_WIDTH")" \
    FUZZEL_INTERNAL_BORDER_RADIUS "$(desktop_shell_config_value "$FUZZEL_INTERNAL_BORDER_RADIUS")" \
    FUZZEL_EXTERNAL_FONT_SIZE "$(desktop_shell_config_value "$FUZZEL_EXTERNAL_FONT_SIZE")" \
    FUZZEL_EXTERNAL_HORIZONTAL_PADDING "$(desktop_shell_config_value "$FUZZEL_EXTERNAL_HORIZONTAL_PADDING")" \
    FUZZEL_EXTERNAL_VERTICAL_PADDING "$(desktop_shell_config_value "$FUZZEL_EXTERNAL_VERTICAL_PADDING")" \
    FUZZEL_EXTERNAL_INNER_PADDING "$(desktop_shell_config_value "$FUZZEL_EXTERNAL_INNER_PADDING")" \
    FUZZEL_EXTERNAL_LINE_HEIGHT "$(desktop_shell_config_value "$FUZZEL_EXTERNAL_LINE_HEIGHT")" \
    FUZZEL_EXTERNAL_BORDER_WIDTH "$(desktop_shell_config_value "$FUZZEL_EXTERNAL_BORDER_WIDTH")" \
    FUZZEL_EXTERNAL_BORDER_RADIUS "$(desktop_shell_config_value "$FUZZEL_EXTERNAL_BORDER_RADIUS")" \
    FUZZEL_DEFAULT_FONT_SIZE "$(desktop_shell_config_value "$FUZZEL_DEFAULT_FONT_SIZE")" \
    FUZZEL_DEFAULT_HORIZONTAL_PADDING "$(desktop_shell_config_value "$FUZZEL_DEFAULT_HORIZONTAL_PADDING")" \
    FUZZEL_DEFAULT_VERTICAL_PADDING "$(desktop_shell_config_value "$FUZZEL_DEFAULT_VERTICAL_PADDING")" \
    FUZZEL_DEFAULT_INNER_PADDING "$(desktop_shell_config_value "$FUZZEL_DEFAULT_INNER_PADDING")" \
    FUZZEL_DEFAULT_LINE_HEIGHT "$(desktop_shell_config_value "$FUZZEL_DEFAULT_LINE_HEIGHT")" \
    FUZZEL_DEFAULT_BORDER_WIDTH "$(desktop_shell_config_value "$FUZZEL_DEFAULT_BORDER_WIDTH")" \
    FUZZEL_DEFAULT_BORDER_RADIUS "$(desktop_shell_config_value "$FUZZEL_DEFAULT_BORDER_RADIUS")" \
    FUZZEL_LAUNCHER_INTERNAL_WIDTH "$(desktop_shell_config_value "$FUZZEL_LAUNCHER_INTERNAL_WIDTH")" \
    FUZZEL_LAUNCHER_INTERNAL_LINES "$(desktop_shell_config_value "$FUZZEL_LAUNCHER_INTERNAL_LINES")" \
    FUZZEL_LAUNCHER_EXTERNAL_WIDTH "$(desktop_shell_config_value "$FUZZEL_LAUNCHER_EXTERNAL_WIDTH")" \
    FUZZEL_LAUNCHER_EXTERNAL_LINES "$(desktop_shell_config_value "$FUZZEL_LAUNCHER_EXTERNAL_LINES")" \
    FUZZEL_LAUNCHER_DEFAULT_WIDTH "$(desktop_shell_config_value "$FUZZEL_LAUNCHER_DEFAULT_WIDTH")" \
    FUZZEL_LAUNCHER_DEFAULT_LINES "$(desktop_shell_config_value "$FUZZEL_LAUNCHER_DEFAULT_LINES")" \
    FUZZEL_MENU_INTERNAL_WIDTH "$(desktop_shell_config_value "$FUZZEL_MENU_INTERNAL_WIDTH")" \
    FUZZEL_MENU_INTERNAL_LINES "$(desktop_shell_config_value "$FUZZEL_MENU_INTERNAL_LINES")" \
    FUZZEL_MENU_EXTERNAL_WIDTH "$(desktop_shell_config_value "$FUZZEL_MENU_EXTERNAL_WIDTH")" \
    FUZZEL_MENU_EXTERNAL_LINES "$(desktop_shell_config_value "$FUZZEL_MENU_EXTERNAL_LINES")" \
    FUZZEL_MENU_DEFAULT_WIDTH "$(desktop_shell_config_value "$FUZZEL_MENU_DEFAULT_WIDTH")" \
    FUZZEL_MENU_DEFAULT_LINES "$(desktop_shell_config_value "$FUZZEL_MENU_DEFAULT_LINES")" \
    ...
    LABWC_CURSOR_THEME "$(desktop_shell_config_value "${LABWC_CURSOR_THEME}")" \
    LABWC_CURSOR_SIZE "$(desktop_shell_config_value "${LABWC_CURSOR_SIZE:-24}")" \
    LABWC_DESKTOP_GTK_THEME "$(desktop_shell_config_value "${LABWC_DESKTOP_GTK_THEME}")" \
    ...
    LABWC_DESKTOP_GTK_ICON_THEME "$(desktop_shell_config_value "${LABWC_DESKTOP_GTK_ICON_THEME}")" \
    LABWC_DESKTOP_QT_PLATFORMTHEME "$(desktop_shell_config_value "${LABWC_DESKTOP_QT_PLATFORMTHEME}")" \
    LABWC_DESKTOP_QT_STYLE_OVERRIDE "$(desktop_shell_config_value "${LABWC_DESKTOP_QT_STYLE_OVERRIDE}")" \
...
desktop_render_labwc_rc_xml() {
  ...
  desktop_render_role_target_template_deferred \
    ...
    LABWC_DESKTOP_GTK_ICON_THEME "$(desktop_xml_attribute_escape "${LABWC_DESKTOP_GTK_ICON_THEME}")" \
...
# Geometry keys/ranges are shared by validation and rendering. This metadata
# is consumed only by the installer; runtime reads native Waybar JSON and CSS.
desktop_waybar_geometry_schema() {
  if [ ! -s "$TMP_ENV_DIR/waybar-geometry.tsv" ]; then
    fetch_hook "$(installer_repo_join_var DIR_SCRIPTS_DESKTOP waybar-geometry.tsv)" "$TMP_ENV_DIR/waybar-geometry.tsv"
  fi
}

desktop_waybar_geometry_validate() (
  desktop_waybar_geometry_schema || exit 1
  for wb_class in INTERNAL EXTERNAL; do
    while IFS="$(printf '\t')" read -r wb_key wb_kind wb_min wb_max; do
      case "$wb_key" in ''|'#'*) continue ;; *[!A-Z0-9_]*) desktop_fatal "invalid Waybar geometry key" ;; esac
      wb_name="LABWC_WAYBAR_${wb_class}_${wb_key}"
      eval "wb_value=\${${wb_name}-}"
      if [ "$wb_kind" = uint ]; then
        case "$wb_value" in ''|*[!0-9]*|0[0-9]*) desktop_fatal "Waybar geometry must use canonical decimal integers: $wb_name" ;; esac
        [ "${#wb_value}" -le 4 ] || desktop_fatal "Waybar geometry exceeds its numeric budget: $wb_name"
        desktop_validate_uint_range "$wb_name" "$wb_value" "$wb_min" "$wb_max"
      elif [ "$wb_kind" = px-tuple ]; then
        # Length tuples contain fixed numbers of non-negative pixel lengths,
        # never arbitrary CSS or a value inherited from the other output class.
        printf '%s\n' "$wb_value" | awk -v count="$wb_min" -v maximum="$wb_max" '
          NF != count { exit 1 }
          { for (i=1; i<=NF; i++) {
              if ($i !~ /^(0|[1-9][0-9]*)px$/) exit 1;
              sub(/px$/, "", $i); if ($i + 0 > maximum) exit 1
            } }
          END { if (NR != 1) exit 1 }
        ' || desktop_fatal "invalid Waybar geometry: $wb_name"
      else
        desktop_fatal "unknown Waybar geometry type: $wb_kind"
      fi
    done < "$TMP_ENV_DIR/waybar-geometry.tsv"
  done
  while IFS="$(printf '\t')" read -r wb_key wb_kind wb_min wb_max; do
    case "$wb_key" in
      MENU_BUTTON_*) continue ;;
      MENU_*|TOOLTIP_*|POPUP_*) ;;
      *) continue ;;
    esac
    eval "wb_internal=\${LABWC_WAYBAR_INTERNAL_${wb_key}-}"
    eval "wb_external=\${LABWC_WAYBAR_EXTERNAL_${wb_key}-}"
    [ "$wb_internal" = "$wb_external" ] ||
      desktop_fatal "native Waybar GTK popup geometry must match INTERNAL/EXTERNAL: $wb_key"
  done < "$TMP_ENV_DIR/waybar-geometry.tsv"
)

# One literal renderer for config, CSS and native menus. Schema validation is
# mandatory before any geometry reaches JSON, CSS or GtkBuilder XML.
desktop_render_waybar_asset() (
  wb_asset=$1
  shift
  case "$wb_asset" in
    config|style.css|audio-menu.xml|calendar-menu.xml|notifications-menu.xml|power-menu.xml|tomat-menu.xml) ;;
    *) desktop_fatal "unsupported Waybar asset: $wb_asset" ;;
  esac
  desktop_waybar_geometry_validate
  for wb_class in INTERNAL EXTERNAL; do
    while IFS="$(printf '\t')" read -r wb_key wb_kind wb_min wb_max; do
      case "$wb_key" in ''|'#'*) continue ;; esac
      wb_name="LABWC_WAYBAR_${wb_class}_${wb_key}"
      eval "wb_value=\${${wb_name}-}"
      set -- "$@" "$wb_name" "$wb_value"
    done < "$TMP_ENV_DIR/waybar-geometry.tsv"
  done
  desktop_render_role_target_template \
    "etc/skel-desktop/.config/waybar/${wb_asset}.tmpl" \
    "/etc/skel-desktop/.config/waybar/${wb_asset}" 0644 "$@"
)

desktop_render_waybar_config() (
  # Do not let a failed command substitution turn into an empty JSON selector.
  wb_internal_outputs=$(desktop_waybar_internal_outputs_json) || desktop_fatal "cannot render internal Waybar outputs"
  wb_external_outputs=$(desktop_waybar_external_outputs_json) || desktop_fatal "cannot render external Waybar outputs"
  wb_modules_left=$(desktop_waybar_modules_left_json) || desktop_fatal "cannot render Waybar left modules"
  wb_modules_right=$(desktop_waybar_modules_right_json) || desktop_fatal "cannot render Waybar right modules"
  desktop_render_waybar_asset config \
    LABWC_WAYBAR_INTERNAL_OUTPUTS "$wb_internal_outputs" \
    LABWC_WAYBAR_EXTERNAL_OUTPUTS "$wb_external_outputs" \
    LABWC_WAYBAR_MODULES_LEFT "$wb_modules_left" \
    LABWC_WAYBAR_MODULES_RIGHT "$wb_modules_right" \
  ...
  desktop_log "rendered_waybar_config bars=internal,external processes=1"
)

desktop_render_waybar_style() {
  desktop_render_waybar_asset style.css
}
...
desktop_render_gtkgreet_css() {
  ...
  greeter_background_url="file://${LABWC_GREETER_BACKGROUND_PATH}"
...
desktop_stage_boot_logging() {
  # Offline preparation only; never read the installer's journal or start units.
  desktop_stage_role_asset usr/local/libexec/journal-snapshot.tmpl /usr/local/libexec/journal-snapshot 0755
  desktop_stage_role_asset etc/systemd/system/boot-log.service.tmpl /etc/systemd/system/boot-log.service 0644
  desktop_stage_role_asset etc/systemd/system/boot-log.timer.tmpl /etc/systemd/system/boot-log.timer 0644
  run_in_target "prepare persistent boot journal directory" \
    /usr/local/libexec/journal-snapshot --prepare
  stage_target_systemd_unit_enabled boot-log.timer system
}
...
' sh "$ACCOUNT_USERNAME"
  ...
  stage_target_asset \
    ...
    0644
  desktop_stage_role_asset etc/rsyslog.d/19-power.conf.tmpl /etc/rsyslog.d/19-power.conf 0644
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/rsyslog.d/21-apps.conf)" \
    /etc/rsyslog.d/21-apps.conf 0644
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET usr/local/libexec/rsyslog-size-rotate)" \
    /usr/local/libexec/rsyslog-size-rotate 0755
  stage_target_asset \
  ...
  stage_target_asset \
    ...
    0644
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/rsyslog.d/31-apparmor-modes.conf)" \
    /etc/rsyslog.d/31-apparmor-modes.conf \
    0644
  stage_target_asset \
  ...
  stage_target_asset \
    ...
    0644
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/rsyslog.d/43-desktop-apps.conf)" \
    /etc/rsyslog.d/43-desktop-apps.conf \
    0644
  stage_target_asset \
  ...
  stage_target_asset \
    ...
    0644
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/logrotate.d/apparmor-modes)" \
    /etc/logrotate.d/apparmor-modes \
    0644
  stage_target_asset \
  ...
  stage_target_asset \
    ...
    0644
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/logrotate.d/desktop-apps)" \
    /etc/logrotate.d/desktop-apps \
    0644
  stage_target_asset \
  ...
  stage_target_asset \
    ...
    0644
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET usr/local/libexec/rsyslog-security-socket)" \
    /usr/local/libexec/rsyslog-security-socket \
    0755
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/systemd/system/rsyslog.service.d/30-security-scanner-socket.conf)" \
    /etc/systemd/system/rsyslog.service.d/30-security-scanner-socket.conf \
    0644
  desktop_stage_role_asset \
  ...
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/rsyslog.d/44-components.conf)" \
    /etc/rsyslog.d/44-components.conf \
    0644
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/tmpfiles.d/59-log-layout.conf)" \
    /etc/tmpfiles.d/59-log-layout.conf \
    0644
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/systemd/system/rsyslog.service.d/20-log-layout.conf)" \
    /etc/systemd/system/rsyslog.service.d/20-log-layout.conf \
    0644
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET usr/local/libexec/journal-check)" \
    /usr/local/libexec/journal-check \
    0755
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET usr/local/libexec/journal-sealing)" \
    /usr/local/libexec/journal-sealing \
    0755
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/systemd/system/journal-sealing.service)" \
    /etc/systemd/system/journal-sealing.service \
    0644
  stage_target_systemd_unit_enabled journal-sealing.service system
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET usr/local/libexec/log-layout)" \
  ...
  stage_target_asset \
    ...
    0755
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/tmpfiles.d/58-native-logs.conf)" \
    /etc/tmpfiles.d/58-native-logs.conf \
    0644
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/rkhunter.d/99-logging.conf)" \
    /etc/rkhunter.d/99-logging.conf \
    0644
  for managed_logging_unit in clamav-freshclam clamav-daemon; do
    stage_target_asset \
      "$(installer_repo_join_var DIR_HOOKS_TARGET "etc/systemd/system/${managed_logging_unit}.service.d/40-logging.conf")" \
      "/etc/systemd/system/${managed_logging_unit}.service.d/40-logging.conf" \
      0644
  done
  stage_target_asset \
    "$(installer_repo_join_var DIR_HOOKS_TARGET etc/apparmor.d/local/usr.sbin.clamd)" \
    /etc/apparmor.d/local/usr.sbin.clamd \
    0644
  run_in_target "retire byte-exact obsolete R3 logging rules" \
    /usr/local/libexec/native-logging --retire-r3
  run_in_target "configure native package logging without changing other settings" \
    /usr/local/libexec/native-logging --configure
  run_in_target "preflight protected logging destinations before tmpfiles" \
    /usr/local/libexec/log-layout --preflight

  run_in_target "validate effective managed journald policy" \
    /usr/local/libexec/journal-check
  run_in_target "create protected managed logging destinations" \
    /usr/bin/systemd-tmpfiles --create \
    /etc/tmpfiles.d/58-native-logs.conf \
    /etc/tmpfiles.d/59-log-layout.conf \
    /etc/tmpfiles.d/60-security-logs.conf \
    /etc/tmpfiles.d/65-audit-syslog.conf
  run_in_target "validate managed log layout and legacy auth link" \
    /usr/local/libexec/log-layout
  desktop_stage_boot_logging
  ...
  desktop_log "staged_logging_policy journal_input=imuxsock apps=/var/log/managed/apps/apps.log system=/var/log/managed/system/system.log kernel=/var/log/managed/system/kernel.log auth=/var/log/managed/security/auth/auth.log apparmor=/var/log/managed/security/apparmor/apparmor.log rotation=writer-size threshold_bytes=2097152 archives=4 scanner_socket=/run/rsyslog/security-scanners/scanner.sock log_group=adm"
}
