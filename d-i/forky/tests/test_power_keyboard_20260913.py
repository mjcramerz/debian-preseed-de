...
def block(filename, name):
    text = payload_read_text(TARGET / 'etc/apparmor.d' / filename)
    if name in {'firstboot', 'secondboot-cleanup', 'crowdsec-firstboot', 'tailscale-up', 'tpm2-enroll'}:
        text = payload_read_text(TARGET.parents[1] / 'scripts/firstboot/assets/etc/apparmor.d/firstboot.tmpl')
...
''')
        ...
        action = payload_read_text(TARGET / 'usr/local/bin/labwc-system-action')
        helper = self.script('labwc-system-action', action)
        keyboard = payload_read_text(TARGET / 'usr/local/bin/labwc-keyboard-layout')
        keyboard = keyboard.replace('/usr/local/bin/labwc-system-action', str(helper))
        keyboard = keyboard.replace('/etc/labwc/desktop.conf', str(self.base / 'no-defaults'))
    ...
    def invoke(self, *args):
        return subprocess.run(payload_installed_argv([str(self.keyboard), *args]), env=self.env,
    ...
    def test_repeated_clicks_alternate_and_refresh_only_after_reconfigure(self):
        for expected in ('se', 'us', 'se', 'us'):
            ...
            self.assertEqual(payload_read_text(self.state).strip(), expected)
        lines = payload_read_text(self.log).splitlines()
        ...
        self.assertNotIn('UNEXPECTED-TERMINAL', payload_read_text(self.log))

    def test_overlapping_clicks_are_queued_not_discarded(self):
        self.env['RECONFIGURE_DELAY'] = '0.08'
        children = [subprocess.Popen(payload_installed_argv([str(self.keyboard), 'waybar-toggle']), env=self.env,
        ...
        try:
            ...
            self.assertEqual(payload_read_text(self.state).strip(), 'us')
            self.assertEqual(payload_read_text(self.log).count('refresh:'), 18)
        finally:
            for child in children:
    ...
    def test_reconfigure_failure_rolls_back_without_success_refresh(self):
        ...
        self.assertEqual(payload_read_text(self.state).strip(), 'us')
        self.assertNotIn('refresh:', payload_read_text(self.log))
    ...
    def test_both_waybar_definitions_use_completion_driven_refresh(self):
        ...
        result = subprocess.run(payload_installed_argv(['/bin/sh', str(fixture), str(TARGET.parents[3]), str(rendered)]),
                                capture_output=True, text=True, timeout=15)
        self.assertEqual(result.returncode, 0, result.stderr)
        configurations = json.loads(payload_read_text(rendered / 'etc/skel-desktop/.config/waybar/config'))
    ...
    def test_refresh_rejects_extra_arguments_without_terminal(self):
        result = subprocess.run(payload_installed_argv([str(self.base / 'labwc-system-action'),
...
class PanelTests(unittest.TestCase):
    def test_only_dock_uses_the_supervisor(self):
        ...
        with mock.patch.object(panel.os, 'getuid', return_value=1000), \
            ...
            self.assertEqual(panel.main(['waybar']), 2)
            supervise.assert_not_called()
            panel.main(['crystal-dock'])
            supervise.assert_called_once_with(['/usr/bin/crystal-dock'], panel.MESSAGES['crystal-dock'])
...
''')
            ...
            code = ("import runpy,sys; m=runpy.run_path(sys.argv[1]); "
                    ...
                    "signals=m['SIGNALS']+tuple(m['signal'].SIGRTMIN+i for i in (7,8,9))))")
            process = subprocess.Popen(payload_installed_argv([sys.executable, '-B', '-u', '-c', code, str(path), str(child)]),
                                       stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            def line():
...
class SessionPreparationTests(unittest.TestCase):
    ...
    def test_empty_desktop_has_no_close_request_or_wait(self):
        ...
        self.assertTrue(payload_source_exists(self.state / 'resume.json'))
    ...
    def test_windowless_background_and_nested_services_do_not_block(self):
        ...
        self.session.prepare(self.state, self.runtime)
        self.sleep.assert_called_once_with(0.3)
        self.assertTrue(payload_source_exists(self.runtime / 'labwc-session-closing'))
    ...
    def test_unresolved_save_window_cancels_before_teardown(self):
        ...
        self.assertFalse(payload_source_exists(self.runtime / 'labwc-session-closing'))
        self.assertFalse(payload_source_exists(self.state / 'resume.json'))
...
class PowerWorkerTests(unittest.TestCase):
    ...
    def test_template_acknowledges_before_frontend_returns(self):
        ...
        root = payload_read_text(TARGET / 'usr/local/libexec/labwc-admin-action-root')
        self.assertIn('exec /usr/bin/systemctl start "$worker_unit"', root)
        self.assertNotIn('exec /usr/bin/systemctl --no-block', root)
        menu = payload_read_text(TARGET / 'usr/local/bin/labwc-power-menu')
        settings = payload_read_text(TARGET / 'usr/local/bin/labwc-power-settings')
        ...
        logout = payload_read_text(TARGET / 'usr/local/libexec/labwc-logout-root')
...
class AuditPolicyTests(unittest.TestCase):
    def test_every_supplied_complain_event_has_a_scoped_source_fix(self):
        fixture = json.loads(payload_read_text(FIXTURE))
        firstboot = block('system-wrappers', 'firstboot')
        desktop = payload_read_text(TARGET / 'etc/apparmor.d/abstractions/wrapper-desktop')
        policies = {'firstboot': firstboot,
                    'desktop-launcher': block('desktop-utilities', 'desktop-launcher'),
                    'session-controls': block('labwc-session', 'session-controls'),
                    'waybar': block('labwc-session', 'waybar'),
                    'labwc-bluetooth': block('desktop-wrappers', 'labwc-bluetooth') + desktop,
                    'labwc-capture': block('desktop-wrappers', 'labwc-capture') + desktop}
        self.assertEqual(fixture['events'], 329)
        self.assertEqual(sum(r['count'] for r in fixture['records']), fixture['events'])
        for record in fixture['records']:
            with self.subTest(record=record):
                # Historical audit records retain the identity emitted on that host.
                profile = record['profile'].split('//null-', 1)[0].removeprefix('x-')
                policy = policies[profile]
                if record['operation'] == 'ptrace':
                    self.assertIn('ptrace (read) peer=' + record['peer'].removeprefix('x-') + ',', policy)
                    self.assertIn('ptrace (readby) peer=session-controls,',
                                  block('labwc-session', 'labwc-compositor'))
                    continue
                requested = set(record['mask'].replace('c', 'w').replace('a', 'w'))
                granted = set()
                # Preserve historical bytes; translate the one renamed executable
                # identity before checking the current confined policy.
                current_name = {'/usr/local/libexec/x-nvidia-char-links':
                                '/usr/local/libexec/nvidia-char-links'}.get(record['name'], record['name'])
                for pattern, modes in source_file_rules(policy):
                    if pattern.fullmatch(current_name):
                        granted.update(modes)
                self.assertTrue(requested <= granted, (record, granted))
        self.assertIn('/usr/bin/printf rix,', firstboot)
        self.assertIn('/usr/local/libexec/nvidia-char-links rix,', firstboot)
        self.assertNotIn('profile firstboot//null-', firstboot)

    def test_keyboard_refresh_has_confined_exec_and_user_bus_permissions(self):
        keyboard = block('desktop-wrappers', 'labwc-keyboard-layout')
        self.assertIn('/usr/local/bin/labwc-system-action rPx -> labwc-system-action,', keyboard)
        action = block('desktop-wrappers', 'labwc-system-action')
    ...
    def test_signal_permissions_cover_both_sides_and_kernel_rt_base(self):
        ...
        panel = block('labwc-session', 'labwc-panel-run')
        waybar = block('labwc-session', 'waybar')
        names = ', '.join('rtmin+' + str(int(signal.SIGRTMIN) + offset - 32) for offset in (7, 8, 9))
        self.assertNotIn('peer=waybar,', panel)
        self.assertNotIn('peer=labwc-panel-run,', waybar)
        self.assertIn(f'signal (receive) set=({names}) peer=unconfined,', waybar)

    def test_power_controller_does_not_need_cross_domain_ptrace_or_kill(self):
        worker = block('desktop-wrappers', 'labwc-admin-action-worker')
        self.assertNotIn('ptrace (read)', worker)
        self.assertIn('capability kill,', worker)
        self.assertIn('set=(kill chld) peer=labwc-admin-action-worker', worker)
        self.assertNotIn('signal (send),', worker)
        self.assertIn('/run/systemd/notify w,', worker)
        self.assertIn('signal (send, receive) set=(kill chld) peer=labwc-admin-action-worker,', worker)
