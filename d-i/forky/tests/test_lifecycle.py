...
class FirstbootTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.stage("05-cleanup.sh", payload_read_text(FORKY / "scripts/firstboot/05-cleanup.sh"))
    ...
    def run_wrapper(self) -> subprocess.CompletedProcess[str]:
        return subprocess.run(payload_installed_argv(["/bin/sh", str(self.wrapper)]), env=self.env,
    ...
    def test_success_marker_and_idempotent_rerun(self) -> None:
        ...
        self.assertIn("status=0\n", payload_read_text(marker))
        self.assertEqual(stat.S_IMODE(payload_source_stat(marker).st_mode), 0o600)
        original = payload_source_stat(marker).st_mtime_ns
        self.stage("01-early.sh", "exit 42\n")
        self.assertEqual(self.run_wrapper().returncode, 0)
        self.assertEqual(payload_source_stat(marker).st_mtime_ns, original)
    ...
    def test_failure_is_visible_retryable_and_never_complete(self) -> None:
        ...
        self.assertFalse(payload_source_exists(self.path / "state/complete"))
        self.assertIn("status=1\n", payload_read_text(self.path / "logs/status.env"))
        self.stage("04-validation.sh", "exit 0\n")
        self.assertEqual(self.run_wrapper().returncode, 0)
        self.assertTrue(payload_source_exists(self.path / "state/complete"))
    ...
    def test_failed_legacy_marker_does_not_suppress_retry(self) -> None:
        ...
        self.assertEqual(self.run_wrapper().returncode, 0)
        self.assertIn("status=0\n", payload_read_text(self.path / "state/complete"))

    def test_cleanup_refuses_unknown_status(self) -> None:
        result = subprocess.run(payload_installed_argv(["/bin/sh", str(self.stages / "05-cleanup.sh")]),
    ...
    def test_concurrent_run_is_locked_and_term_cleans_child(self) -> None:
        ...
        process = subprocess.Popen(payload_installed_argv(["/bin/sh", str(self.wrapper)]), env=self.env,
        ...
        if payload_source_exists(procstat):
            self.assertEqual(payload_read_text(procstat).split(") ", 1)[1][0], "Z")
        self.assertFalse(payload_source_exists(self.path / "state/complete"))
...
"""
    ...
    def shell(self, body: str) -> subprocess.CompletedProcess[str]:
        return subprocess.run(payload_installed_argv(["/bin/sh", "-c", self.preamble + body]),
    ...
    def test_custom_target_and_private_parent_mode(self) -> None:
        ...
        self.assertEqual(payload_read_text(parent / "file"), "new content\n")
        self.assertEqual(stat.S_IMODE(payload_source_stat(parent).st_mode), 0o700)
        self.assertEqual(stat.S_IMODE(payload_source_stat(parent / "file").st_mode), 0o640)
    ...
    def test_failed_fetch_preserves_old_file(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(payload_read_text(target), "old content")
    ...
    def test_dotdot_rejected(self) -> None:
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertFalse(payload_source_exists(self.path / "escaped"))
    ...
    def test_target_symlink_replaced_not_followed(self) -> None:
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(payload_read_text(outside), "untouched")
...
class PublishingEntrypointTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.target = self.path / "target"
        self.target.mkdir()
        source = payload_read_text(FORKY / "scripts/late/devops.sh")
...
'''
    ...
    def test_regular_file_collision_is_rejected_without_replacement(self) -> None:
        ...
        self.assertEqual(payload_read_text(link), "unmanaged\n")
...
{function}
"""
        result = subprocess.run(payload_installed_argv(["/bin/sh"]), input=source, text=True,
    ...
    def test_tmpfs_cleanup_preserves_symlink_destination(self) -> None:
        with tempfile.TemporaryDirectory(prefix="pre-clean-") as name:
            ...
            source = payload_read_text(SHARED / "usr/local/libexec/tmpfs-pre-clean.tmpl")
            source = source.replace("__INSTALLER_TMPFS_PRE_CLEAN_TARGETS__", f"test|{clean}")
            result = subprocess.run(payload_installed_argv(["/bin/sh"]), input=source, text=True,
                                    capture_output=True, timeout=5)
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertEqual(list(clean.iterdir()), [])
            self.assertEqual(payload_read_text(outside), "keep")
    ...
    def test_tmpfs_preflight_refuses_mountinfo_descendant(self) -> None:
        with tempfile.TemporaryDirectory(prefix="pre-clean-") as name:
            ...
            result = subprocess.run(payload_installed_argv(["/bin/sh"]), input=source, text=True,
                                    capture_output=True, timeout=5)
            self.assertNotEqual(result.returncode, 0)
            self.assertEqual(payload_read_text(clean / "file"), "keep")

    def test_apt_refresh_does_not_wait_for_persistent_dispatcher_exit(self) -> None:
        unit = payload_read_text(SHARED / "etc/systemd/system/apt-refresh-lists.service.tmpl")
        self.assertNotIn("systemctl is-active", unit)
        self.assertIn("Restart=on-failure", unit)
        self.assertIn("StartLimitBurst=3", unit)
        helper = payload_read_text(SHARED / "usr/local/libexec/apt-refresh-lists.tmpl")
    ...
    def test_firstboot_starts_after_boot_prerequisites_without_waiting_for_diagnostics(self) -> None:
        unit = payload_read_text(SHARED / "etc/systemd/system/firstboot.service")
        self.assertIn("Type=exec", unit)
        self.assertNotIn("Type=oneshot", unit)
        self.assertIn("Wants=network-online.target apparmor-modes.service", unit)
