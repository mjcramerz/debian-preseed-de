...
validate_tmpfs_policy_config() {
  ...
  [ "${DIR_VAR_SPOOL:-}" = /var/spool ] &&
    ...
    fatal "rsyslog spool path must be /var/spool/rsyslog"
  case "${SIZE_TMPFS_VAR_SPOOL_RSYSLOG_MIB:-}" in
    ''|0*|*[!0-9]*) fatal "invalid rsyslog spool tmpfs size" ;;
  esac
  [ "${#SIZE_TMPFS_VAR_SPOOL_RSYSLOG_MIB}" -le 5 ] &&
    [ "$SIZE_TMPFS_VAR_SPOOL_RSYSLOG_MIB" -le 65536 ] ||
    fatal "rsyslog spool tmpfs must be 1..65536 MiB"
  [ -n "${MNT_VAR_SPOOL_RSYSLOG_TMPFS_OPTS:-}" ] ||
    fatal "rsyslog spool tmpfs options are missing"
}
...
DIR_ROOT_HOME DIR_OPT DIR_SRV DIR_USR_LOCAL DIR_VAR_TMP DIR_VAR_SPOOL DIR_VAR_SPOOL_RSYSLOG DIR_HOME DIR_HOME_DOCUMENTS DIR_HOME_DOWNLOADS DIR_HOME_PUBLIC DIR_HOME_PICTURES DIR_HOME_WORKSPACE DIR_HOME_SYNCTHING DIR_DEV_SHM
...
MNT_VAR_SPOOL_RSYSLOG_TMPFS_OPTS MNT_VAR_LOG_TMPFS_OPTS MNT_VAR_CACHE_TMPFS_OPTS MNT_APT_LISTS_TMPFS_OPTS MNT_COREDUMP_TMPFS_OPTS MNT_DATA_RUN_TMPFS_OPTS MNT_TMP_OPTS MNT_DEV_SHM_OPTS
...
TMPFS_VAR_LOG TMPFS_VAR_SPOOL_RSYSLOG TMPFS_VAR_CACHE TMPFS_VAR_LIB_APT_LISTS TMPFS_DEV_SHM TMPFS_DATA_RUN TMPFS_SYSTEMD_COREDUMP"
  missing=0
...
prepare_install_volatile_dirs() {
  ...
  if layout_bool_is_true "$TMPFS_VAR_SPOOL_RSYSLOG"; then
    ensure_dir_mode "$TARGET_ROOT$DIR_VAR_SPOOL_RSYSLOG" 0700
  fi
...
prepare_volatile_dirs() {
  ...
  if layout_bool_is_true "$TMPFS_VAR_SPOOL_RSYSLOG"; then
    install -d -m 0700 "$TARGET_ROOT$DIR_VAR_SPOOL_RSYSLOG"
  fi
  if layout_bool_is_true "$TMPFS_VAR_CACHE"; then
