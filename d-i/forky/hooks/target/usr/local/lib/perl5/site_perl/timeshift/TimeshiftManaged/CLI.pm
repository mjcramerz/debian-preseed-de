...
sub _usage {
    ...
    if ($self->program() eq 'timeshift-snapshot') {
        print STDERR "usage: timeshift-snapshot {daily|weekly|monthly}\n";
    }
    else {
...
sub run {
    ...
    if ($self->program() eq 'timeshift-snapshot') {
        ...
        if (!$snapshot) {
            ...
            $error =~ s/\s+\z//;
            TimeshiftManaged::Logger->new(tag => 'timeshift-snapshot')->error($error);
            return 1;
        }
        return $snapshot->run();
    }
