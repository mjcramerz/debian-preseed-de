package APTRepoLocal::Servicing::Event;
...
sub emit {
    ...
    APTRepoLocal::Servicing::Atomic->ensure_root_directory($self->directory(), 0755);
    my $sequence = $self->sequence() + 1;
    $self->sequence($sequence);
    my $name = sprintf('%010d-%010d-%04d.event', time(), $$, $sequence);
    my $path = APTRepoLocal::Servicing::Atomic->assert_child($self->directory(), $name);
