...
# Runtime publication belongs exclusively to apt-repo-local; it never
# installs packages or invokes APT from an APT update hook.
has directory => (is => 'ro', isa => Str, default => sub { '/var/lib/software/repo' });
use constant HELPER => '/usr/local/libexec/apt-repo-local';
...
sub retain {
    ...
    if ($metadata->{package} eq 'bitwarden') {
        my $digest = APTRepoLocal::Servicing::Atomic->sha256_file($source, 536_870_912);
        APTRepoLocal::Servicing::Atomic->write_text("$path.vendor-sha256", "$digest\n", 0644);
    }
...
sub latest {
    ...
    for my $record (values %{$catalog->{packages}}) {
        ...
        if ($spec->{name} eq 'bitwarden') {
            $digest = APTRepoLocal::Servicing::Atomic->sha256_file($path, MAX_DEB_BYTES);
...
sub bitwarden_vendor_digest_matches {
    ...
    return APTRepoLocal::Servicing::Atomic->read_limited($receipt, 65)
        eq "$digest\n";
}
...
sub repair {
    ...
    for my $package (@packages) {
        ...
        return 0 if APTRepoLocal::Servicing::Process->application_running(
