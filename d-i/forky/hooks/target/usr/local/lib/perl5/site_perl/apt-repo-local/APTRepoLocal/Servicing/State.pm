package APTRepoLocal::Servicing::State;
...
use constant WORK_PREFIX        => 'apt-repo-local-software-update.';
...
sub lock_path { return '/run/lock/apt-repo-local-software-update.lock'; }

sub prepare {
    my ($self) = @_;
    APTRepoLocal::Servicing::Atomic->ensure_root_directory($self->root(), 0755);
    APTRepoLocal::Servicing::Atomic->ensure_root_directory($self->event_dir(), 0755);
    APTRepoLocal::Servicing::Atomic->ensure_root_directory($self->deb_dir(), 0755);
    APTRepoLocal::Servicing::Atomic->ensure_root_directory($self->inbox_dir(), 0700);
    APTRepoLocal::Servicing::Atomic->ensure_root_directory($self->artifact_dir(), 0755);
    APTRepoLocal::Servicing::Atomic->ensure_root_directory($self->state_dir(), 0755);
    return 1;
}
...
sub artifact_directory {
    ...
    APTRepoLocal::Servicing::Atomic->ensure_root_directory($directory, 0755);
    return $directory;
}
...
sub artifact_path {
    ...
    return APTRepoLocal::Servicing::Atomic->assert_child($directory, $name);
}

sub retain_artifact {
    my ($self, $application, $source, $name) = @_;
    APTRepoLocal::Servicing::Atomic->assert_absolute_path('artifact source', $source);
    -f $source && !-l $source
...
sub state_path {
    ...
    return APTRepoLocal::Servicing::Atomic->assert_child($self->state_dir(), $name);
}
...
sub read_state {
    ...
    return APTRepoLocal::Servicing::Atomic->read_limited($path, $limit);
}
...
sub write_state {
    ...
    return APTRepoLocal::Servicing::Atomic->write_text(
...
sub lock {
    ...
    APTRepoLocal::Servicing::Atomic->assert_absolute_path('managed updater lock', $path);
...
sub _assert_work_dir_path {
    my ($self, $path) = @_;
    APTRepoLocal::Servicing::Atomic->assert_absolute_path('updater workspace', $path);
