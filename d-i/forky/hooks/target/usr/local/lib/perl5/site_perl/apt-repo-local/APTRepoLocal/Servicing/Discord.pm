package APTRepoLocal::Servicing::Discord;
...
use constant ARCHIVE_HELPER => '/usr/local/libexec/discord-distro';
use constant INSTALL_ROOT => '/opt/discord';
use constant DESKTOP_FILE => '/usr/local/share/applications/discord.desktop';
...
sub _record_failure_detail {
    ...
    print STDERR "[apt-repo-local-software-update] error: $message\n";
...
sub _assert_runtime {
    ...
    my $managed_release = APTRepoLocal::Servicing::Atomic->read_limited(
        "$root/.release",
        4096,
    );
...
sub _publish {
    ...
    system('/usr/bin/update-desktop-database', '/usr/local/share/applications');
    return (0, 'updated');
}
...
sub apply {
    ...
    return (1, 'in-use') if APTRepoLocal::Servicing::Process->application_running(
